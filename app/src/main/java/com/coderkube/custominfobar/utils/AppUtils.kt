package com.coderkube.custominfobar.utils
...
import com.ckpackage.custominfobar.defaultInfoBars.ErrorInfoBar
import com.ckpackage.custominfobar.defaultInfoBars.SlideToPerformInfoBar
import com.ckpackage.custominfobar.defaultInfoBars.SuccessInfoBar
import com.ckpackage.custominfobar.defaultInfoBars.WarningInfoBar
...
/**
 * Function to create [ComposeInfoBar] based on the pressed button's type.
 *
 * @param type [ButtonType] of the button pressed.
 * @param content [ComposeInfoBarData] which contains title and message for [ComposeInfoBar].
 * @param isInfinite Flag that represents whether the [ComposeInfoBar]'s Duration is infinite or not.
 * @param shape Shape of the [ComposeInfoBar].
 * @param onClose Called when user clicks on clear button on [ComposeInfoBar] (Note: This Clear button will only be visible if isInfinite is true).
 */
@Composable
fun InfoBarByButtonType(
    type: ButtonType,
    content: ComposeInfoBarData,
    isInfinite: Boolean,
    shape: Shape,
    onClose: () -> Unit = {}
) {
...
fun getInfoBarTitle(context: Context, buttonType: ButtonType): TextType {
    return when (buttonType) {
        ...
        ButtonType.ActionInfoBar -> context.getString(R.string.composeinfobar_with_action)
            .toTextType()
...
/**
 * Dummy InfoBar to show [ComposeInfoBar] with action.
 *
 * @param title Title string for the [ComposeInfoBar].
 * @param description Description string for the [ComposeInfoBar].
 * @param isInfinite Flag that decides whether [ComposeInfoBar]'s duration is infinite.
 * @param shape Shape of the [ComposeInfoBar].
 * @param onClose Called when user clicks on the clear button on the [ComposeInfoBar].
 */
@Composable
private fun DummyInfoBarWithAction(
    ...
    ComposeInfoBar(
        ...
        isInfinite = isInfinite,
        shape = shape,
        contentColors = ComposeInfoBarDefaults.colors.copy(
...
/**
 * Dummy Default [ComposeInfoBar] for the demo purposes.
 *
 * @param title Title string for the [ComposeInfoBar].
 * @param description Description string for the [ComposeInfoBar].
 * @param isInfinite Flag that decides whether [ComposeInfoBar]'s duration is infinite.
...
/**
 * Dummy [ComposeInfoBar] with Drawable Background for demo purposes.
 *
 * @param title Title string for the [ComposeInfoBar].
 * @param description Description string for the [ComposeInfoBar].
...
private fun DummyInfoBarWithDrawableBackground(
    ...
    ComposeInfoBar(
        title = title,
        description = description,
        customBackground = background.toCustomBackground(),
...
/**
 * Dummy [ComposeInfoBar] with SVG Background for demo purposes.
 *
 * @param title Title string for the [ComposeInfoBar].
 * @param background of type [Painter].
 * @param description Description string for the [ComposeInfoBar].
...
/**
 * Dummy [ComposeInfoBar] with Gradient Background Using Brush for demo purposes.
 *
 * @param title Title string for the [ComposeInfoBar].
 * @param description Description string for the [ComposeInfoBar].
 * @param background Background of type [Brush] to be applied to [ComposeInfoBar].
 * @param isInfinite Flag that decides whether [ComposeInfoBar]'s duration is infinite.
 * @param shape Shape of the [ComposeInfoBar].
 * @param onClose Called when user clicks on the clear button on the [ComposeInfoBar].
 */
...
/**
 * Extension function on [CoroutineScope] to show [ComposeInfoBar].
 *
 * @param title title of the [ComposeInfoBar].
 * @param description description of the [ComposeInfoBar].
 * @param composeInfoHostState [ComposeInfoHostState] used to show the [ComposeInfoBar].
 * @param duration duration for the [ComposeInfoBar].
 * @param context additional to CoroutineScope.coroutineContext context of the coroutine.
 * @param start coroutine start option. The default value is CoroutineStart.DEFAULT.
 *
 * @return [Job] to control the coroutine.
 */
fun CoroutineScope.showComposeInfoBar(
    title: TextType,
    description: TextType? = null,
    ...
    composeInfoHostState.show(
        infoBarData = ComposeInfoBarData(title, description, icon),
        duration = duration
    )
}